...
- 現在時刻が、Valueに記載された有効期間内になっていること
...
**エラー処理**:
- メール送信失敗時は内部エラーとして記録
- 無効なトークンや期限切れの場合は適切なエラーメッセージを表示
  - 無効なトークン(KVストアにトークンが存在しない)
  - 期限切れ(KVストアにトークンは存在するが、期限が切れている)
...
**概要**:
現状のアルゴリズムでは、利用されなかったトークンやセッションIDはそのままKVストアに残りつづけてしまいます。KVストアが無効なデータで肥大化するのを防ぐために、定期的にKVストアを読み出して無効な値を削除します。

**提供する機能**:
- **無効な認証トークン及びセッションID削除**: 全てのKeyに対して、Valueのexpires_atを読み出し、現時点で無効な値は削除する
- **レート制限期間が完全に切れたレート制限削除**: 全てのKeyに対して、Valueのrequested_atを読み出し、現時点でレート制限のかかっていない値は削除する
