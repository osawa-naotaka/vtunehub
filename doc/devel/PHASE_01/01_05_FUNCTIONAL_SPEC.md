...
#### PublicStreams - 静的サイトデプロイ時のJsonファイルの型
```typescript
interface PublicStreams PublicStream[];
...
**認証トークンデータ構造**: AuthToken型 (2.3.2 認証トークン 参照)
...
**セッションデータ構造**: SessionData型 (2.3.2 セッションデータ 参照)
...
**概要**:
各システムからのAPIリクエストに応答します。APIではドメインロジックを実装します。主にスケジュールのCRUD機能があります。
