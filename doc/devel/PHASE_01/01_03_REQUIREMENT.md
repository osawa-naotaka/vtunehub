...
- **Durable Object**: Cloudflare Durable Object (分散永続オブジェクト)
- **Cache**: Cloudflare Cache API (キャッシュ)
...
- 分散永続オブジェクト - Cloudflare Durable Object (レート制限)
...
##### FR-6: リスナー閲覧用の静的サイト機能
- 優先度: 必須
- 説明: スケジュールを閲覧できる静的サイトを運用できること
- 詳細:
  - 静的サイトとして運用し、運用コストを低減する
  - SPAとして構築し、閲覧者全員が共通のhtmlファイルを読み込む
  - SPAが読み込まれるたびにスケジュール書き込まれたJsonファイルをフェッチする
    - Jsonファイルは適宜Workersで生成される
    - Cache APIを利用して、DBアクセス回数を削減する
...
| D1パフォーマンス問題 | 中 | 中 | インデックス最適化、DO/キャッシュ |
